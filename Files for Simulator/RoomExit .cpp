...
RoomExit::RoomExit(std::vector<Eigen::Vector3d> &data) : points(data) {
    ...
    xyzFile.open("RoomMap.xyz");
    for (auto &point: points) {
        xyzFile << point.x() << "," << point.y() << "," << point.z() << std::endl;
    }
    xyzFile.close();
    ...
    std::string filename = "/home/ben/dev/simulatorMapping/tools/navigation/main.py";
    //std::string tempFile = "RoomMap.xyz";
    //std::string command = "python " + filename + " " + xyzFile;
    std::string command = "python3 " + filename + " " + "RoomMap.xyz";
    struct xyz {
        double x;
        double y;
    };
    FILE *exit = popen(command.c_str(),"r");
    if (exit == nullptr) {
        std::cout << "Error: " << strerror(errno);
        return ;
    }
    while (fgets(buffer, sizeof(buffer), exit) != NULL) {
        result += buffer;
    }
    //Find substring that starts and ends with $
    std::string delimiter = "$";
    std::string token = result.substr(result.find(delimiter) + 1, result.find(delimiter, result.find(delimiter) + 1) - 1);
    std::cout << token << std::endl;
    //Separate the string into x and y
    std::string delimiter2 = ",";
    std::string x = token.substr(0, token.find(delimiter2));
    std::string y = token.substr(token.find(delimiter2) + 1, token.length());
    double x1 = std::stod(x);
    double y1 = std::stod(y);
    double z1 = 0;
    std::cout << x1 << "," << y1 << "," << z1 << std::endl;
    Eigen::Vector3d exitPoint(x1, y1, z1);
    RoomExit::exit = exitPoint;
}
...
//     calculateVariances();
//     filterByVariance();
//     return RoomExit::exit;
// }

Eigen::Vector3d RoomExit::getExitPoints() {
    getPolygonVertices();
    calculateVariances();
    filterByVariance();
    return exit;
